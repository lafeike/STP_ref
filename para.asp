...
%>	
	...
	<div data-role="content" class="jqm-content">
		...
		<% 
		SQL = "select paraNum, question, guidenote from tblrbParagraph p " &_
			  "where paraNum = " & para & " AND p.rbSectKey = " & rbSec
			set rsParagraph = conn_pub.execute(SQL)

		if not (rsParagraph.eof and rsParagraph.bof) then 
			%>
				<p><a name="<%=rsParagraph("paraNum")%>"></a><b><%=rsParagraph("paraNum")%></b> &nbsp; <%=removeHTML(HTMLDecode(rsParagraph("question")))%></p>
				<p><%=removeLinks(HTMLDecode(rsParagraph("guidenote")))%></ul></ul></ul></p>
			...
			if len(session("state")) > 0 then
				...
				if not(getSD.eof and getSD.bof) then
					while not getSD.eof
						...
						<li <% if para = getSD("sdKey") then %>data-theme="b"<% end if %>>
